...
public class OrderClient extends HttpClient {
    ...
    public Response create(Order order) {
        return sendRequest(getCommonReqSpec()
                .baseUri(STORE_URL + "/" + "order")
                .body(order), Method.POST);
    }

    public Response getInventory() {
        return sendRequest(getCommonReqSpec()
                .baseUri(STORE_URL + "/" + "inventory"), Method.GET);
    }

    public Response getOrderById(Long orderId) {
        return sendRequest(getCommonReqSpec()
                .baseUri(STORE_ORDER_URL + orderId), Method.GET);
    }

    public Response deleteOrderById(Long orderId) {
        return sendRequest(getCommonReqSpec()
                .baseUri(STORE_ORDER_URL + orderId), Method.DELETE);
    }
}
