...
public class PetClient extends HttpClient {
    ...
    public Response create(Pet pet) {
        return sendRequest(getCommonReqSpec()
                .baseUri(PET_URL).body(pet), Method.POST);
    }

    public Response get(Pet pet) {
        return sendRequest(getCommonReqSpec()
                .baseUri(PET_URL + "/" + pet.getId()), Method.GET);
    }

    public Response delete(Pet pet) {
        return sendRequest(getCommonReqSpec().headers(apiKeyHeader)
                .baseUri(PET_URL + "/" + pet.getId()), Method.DELETE);
    }

    public Response update(Pet pet) {
        return sendRequest(getCommonReqSpec()
                .baseUri(PET_URL).body(pet), Method.PUT);
    }
}
